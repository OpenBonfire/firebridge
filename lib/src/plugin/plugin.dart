...
abstract class FirebridgePlugin<ClientType extends Firebridge> {
  ...
  late final Expando<
          FirebridgePluginState<ClientType, FirebridgePlugin<ClientType>>>
  ...
  FutureOr<FirebridgePluginState<ClientType, FirebridgePlugin<ClientType>>>
      createState() => FirebridgePluginState(this);
  ...
  /// {@endtemplate}
  @mustCallSuper
  Stream<ShardMessage> interceptShardMessages(
          Shard shard, Stream<ShardMessage> messages) =>
      _states[shard.client]?.interceptShardMessages(shard, messages) ??
      messages;
  ...
  /// {@endtemplate}
  @mustCallSuper
  Stream<GatewayMessage> interceptGatewayMessages(
          Shard shard, Stream<GatewayMessage> messages) =>
      _states[shard.client]?.interceptGatewayMessages(shard, messages) ??
      messages;
}

/// Holds the state of a plugin added to a client.
class FirebridgePluginState<ClientType extends Firebridge,
    PluginType extends FirebridgePlugin<ClientType>> {
  ...
  FirebridgePluginState(this.plugin);
