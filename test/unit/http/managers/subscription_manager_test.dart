...
void checkSubscription(Subscription subscription) {
  ...
  expect(subscription.currentPeriodStart,
      equals(DateTime.utc(2024, 08, 27, 19, 48, 44, 406, 602)));
  expect(subscription.currentPeriodEnd,
      equals(DateTime.utc(2024, 09, 27, 19, 48, 44, 406, 602)));
...
void main() {
  testReadOnlyManager<Subscription, SubscriptionManager>(
    'SubscriptionManager',
    (client, config) => SubscriptionManager(client, config,
        applicationId: Snowflake.zero, skuId: Snowflake(1)),
    RegExp(r'/skus/1/subscriptions/\d+'),
    sampleObject: sampleSubscription,
    sampleMatches: checkSubscription,
    additionalParsingTests: [],
    additionalEndpointTests: [
      EndpointTest<SubscriptionManager, List<Subscription>,
