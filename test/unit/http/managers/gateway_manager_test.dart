...
void checkGateway(GatewayConfiguration configuration) {
  expect(configuration.url,
      equals(Uri(scheme: 'wss', host: 'gateway.discord.gg', path: '/')));
}
...
void checkGatewayBot(GatewayBot gateway) {
  ...
  expect(gateway.sessionStartLimit.resetAfter,
      equals(Duration(milliseconds: 14400000)));
...
void checkActivity2(Activity activity) {
  ...
  expect(activity.createdAt, isNull);
  expect(activity.timestamps?.start,
      equals(DateTime.fromMillisecondsSinceEpoch(15112000660000)));
  ...
  expect(activity.secrets?.join,
      equals('025ed05c71f639de8bfaa0d679d7c94b2fdce12f'));
  expect(activity.secrets?.spectate,
      equals('e7eb30d2ee025ed05c71ea495f770b76454ee4e0'));
  expect(activity.secrets?.match,
      equals('4b2fdce12f639de8bfa7e3591b71a0d679d7c93f'));
  expect(activity.isInstance, isNull);
  expect(activity.flags, isNull);
  expect(activity.buttons, isNull);
}
